...
import (
	...
	"github.com/argoproj/argo-events/signals/core/stream"
...
type nats struct{
	...
	gatewayConfig *stream.GatewayConfig
...
func (n *nats) listen(s *stream.Stream) {
	...
	_, err = conn.Subscribe(s.Attributes[subjectKey], func(msg *natsio.Msg) {
		...
		http.Post(fmt.Sprintf("http://localhost:%s", n.transformerPort), "application/octet-stream", bytes.NewReader(msg.Data))
	})
	if err != nil {
...
CheckAlreadyRegistered:
	for scheduleKey, scheduleValue := range cm.Data {
		var s *stream.Stream
		err := yaml.Unmarshal([]byte(scheduleValue), &s)
		if err != nil {
			n.gatewayConfig.Log.Error().Str("artifact", scheduleKey).Err(err).Msg("failed to parse calendar schedule")
			return err
		}
...
func main() {
	...
	_, err = n.gatewayConfig.WatchGatewayConfigMap(n, context.Background(), configName)
	if err != nil {
