	...
	return &operationContext{
		...
		logger: common.NewArgoEventsLogger().WithFields(
			map[string]interface{}{
				common.LabelGatewayName: gatewayObj.Name,
...
func (opctx *operationContext) updateGatewayState() {
	if opctx.updated {
		...
		labels := map[string]string{
			common.LabelGatewayName:                    opctx.gatewayObj.Name,
			common.LabelGatewayKeyPhase:                string(opctx.gatewayObj.Status.Phase),
			common.LabelKeyGatewayControllerInstanceID: opctx.controller.Config.InstanceID,
...
func (opctx *operationContext) markGatewayPhase(phase v1alpha1.NodePhase, message string) {
	...
	if justCompleted {
		...
		opctx.gatewayObj.ObjectMeta.Labels[common.LabelGatewayKeyPhase] = string(phase)
		// add annotations so a resource sensor can watch this gateway.
		opctx.gatewayObj.Annotations[common.LabelGatewayKeyPhase] = string(phase)
	}
