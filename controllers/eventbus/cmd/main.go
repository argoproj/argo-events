...
func main() {
	...
	if err != nil {
		logger.WithError(err).Errorf("unable to add scheme")
		panic(err)
	}
	c, err := controller.New(eventbus.ControllerName, mgr, controller.Options{
		Reconciler: eventbus.NewReconciler(mgr.GetClient(), mgr.GetScheme(), images, logger),
	})
	if err != nil {
		logger.WithError(err).Errorf("unable to set up individual controller")
		panic(err)
	}
	...
	if err := c.Watch(&source.Kind{Type: &v1alpha1.EventBus{}}, &handler.EnqueueRequestForObject{}); err != nil {
		logger.WithError(err).Errorf("unable to watch EventBuses")
		panic(err)
	}
	...
	if err := c.Watch(&source.Kind{Type: &appv1.StatefulSet{}}, &handler.EnqueueRequestForOwner{OwnerType: &v1alpha1.EventBus{}, IsController: true}); err != nil {
		logger.WithError(err).Errorf("unable to watch StatefulSets")
		panic(err)
	}

	// Watch Deployments and enqueue owning EventBus key
	if err := c.Watch(&source.Kind{Type: &appv1.Deployment{}}, &handler.EnqueueRequestForOwner{OwnerType: &v1alpha1.EventBus{}, IsController: true}); err != nil {
		logger.WithError(err).Errorf("unable to watch Deployments")
		panic(err)
	}
	...
	if err := c.Watch(&source.Kind{Type: &corev1.Service{}}, &handler.EnqueueRequestForOwner{OwnerType: &v1alpha1.EventBus{}, IsController: true}); err != nil {
		logger.WithError(err).Errorf("unable to watch Services")
		panic(err)
	}

	logger.Info("starting manager")
	if err := mgr.Start(signals.SetupSignalHandler()); err != nil {
		logger.WithError(err).Errorf("unable to run manager")
		panic(err)
	}
}
