...
func Install(eventBus *v1alpha1.EventBus, client client.Client, natsImage, natsStreamingImage string, logger logr.Logger) error {
	installer, err := getInstaller(eventBus, client, natsImage, natsStreamingImage, logger)
	if err != nil {
...
func getInstaller(eventBus *v1alpha1.EventBus, client client.Client, natsImage, natsStreamingImage string, logger logr.Logger) (Installer, error) {
	if nats := eventBus.Spec.NATS; nats != nil {
		if nats.Exotic != nil {
			return NewExoticNATSInstaller(eventBus, logger), nil
		} else if nats.Native != nil {
			return NewNATSInstaller(client, eventBus, natsImage, natsStreamingImage, getLabels(eventBus), logger), nil
		}
	}
...
// separately.
func Uninstall(eventBus *v1alpha1.EventBus, client client.Client, natsImage, natsStreamingImage string, logger logr.Logger) error {
	installer, err := getInstaller(eventBus, client, natsImage, natsStreamingImage, logger)
	if err != nil {
