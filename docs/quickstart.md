...
Note: Modify the [argo-events-cluster-roles.yaml](../hack/k8s/manifests/argo-events-cluster-roles.yaml) file to use the correct namespace that you wish to deploy the gateway controller and gateway controller.
...
Verify that the gateway was updated correctly and moved to a "Complete" phase.
```
kubectl get gateway -n argo-events webhook-gateway -o yaml
```

Check the logs of the gateway-controller pod, gateway-controller, associated gateways and sensors if there are problems.
