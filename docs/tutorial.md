...
1. [What are gateway and gateway controllers](controllers-guide.md)
2. [Learn about gateways](gateway-guide.md)
3. [Learn about sensors](gateway-guide.md)
...
7. [Passing payload from eventDependency to trigger](#passing-payload-from-eventDependency-to-trigger)
8. [Sensor filters](#gateway-filters)
...
3) <h5>Now its time to create webhook gateway.</h5>
    1. Sensor definition,
        ...
        metadata:
          ...
          name: webhook-gateway
          labels:
            # instance-id must match with one of the deployed gateway controller's instance-id
            sensors.argoproj.io/gateway-controller-instanceid: argo-events
        spec:
          # make this gateway as long running.
          repeat: true
          serviceAccountName: argo-events-sa
          # signals/notifications this gateway is interested in.
          signals:
        ...
        This gateway defines only one eventDependency called `webhook-gateway/webhook.fooConfig`, meaning, it is interested in listening
    ...
    3. Check whether all gateway nodes are initialized,
        ```bash
        kubectl get -n argo-events sensors webhook-gateway   
        ```
...
Lets start with deploying Minio server standalone deployment. You can get the K8 deployment from https://www.minio.io/kubernetes.html
   ...
   6. Below is the gateway definition, 
        ```yaml
        apiVersion: argoproj.io/v1alpha1
        kind: Sensor
        metadata:
        ...
        ```bash
        kubectl -n argo-events get sensors artifact-gateway -o yaml
        ```
...
Resource gateway can monitor any K8 resource and any CRD.
 ...
 1. Lets have a look at a configuration,
    ...
    * Gateway configuration `resource.barConfig` will send event to watchers whenever a gateway label `workflows.argoproj.io/phase: Failed` is added.
 ...
 4. Create an basic `hello-world` argo workflow with name  `my-workflow`. 
    ...
    Once workflow is created, resource gateway will trigger workflow.
 ...
 * ### <a name="mqtt">MQTT</a>
   ...
   4. Deploy the gateway,
      ```bash
      kubectl -n argo-events create -f https://raw.githubusercontent.com/argoproj/argo-events/master/examples/sensors/mqtt-gateway.yaml
      ```  
...
## <a name="updating-configurations">Updating gateway configurations dynamically</a>
  ...
  3) Update the webhook gateway or create a new gateway to listen to this new configuration.
...
## <a name="passing-payload-from-eventDependency-to-trigger">Passing payload from eventDependency to trigger</a> 
 ...
 * ### Complete payload
     ...
     1. Create a webhook gateway,
        ...
        kind: Sensor
        metadata:
          name: webhook-with-resource-param-gateway
          labels:
            sensors.argoproj.io/gateway-controller-instanceid: argo-events
        spec:
          repeat: true
              ...
              resource:
                ...
                parameters:
                  - src:
                    ...
                    # pass payload of webhook-gateway/webhook.fooConfig eventDependency to first parameter value
                    # of arguments.
     ...
     2. <b>Note that gateway name is `webhook-with-resource-param-gateway`. Update your gateway accordingly or create a new one.</b>
          ...
          resource:
            ...
            parameters:
              - src:
                  ...
                  # of webhook-gateway/webhook.fooConfig eventDependency
                  path: message
...
## Sensor Filters
 Following are the types of the filter you can apply on eventDependency/event payload,
