...
The structure of an event dispatched by the event-source to the sensor looks like following,

        {
            "context": {
              ...
              "extensions": { /* CloudEvent extension attributes, if present (e.g., traceparent, tracestate) */ }
            },
            "data": {
...
### CloudEvents Passthrough

When the webhook endpoint receives a request that is a valid
[CloudEvent](https://cloudevents.io/) (either in binary content mode with
`Ce-*` headers or in structured content mode with
`Content-Type: application/cloudevents+json`), the original CloudEvent
attributes (`id`, `source`, `type`, `subject`, `time`) and any extension
attributes (e.g., `traceparent`, `tracestate`) are preserved and propagated
through the EventBus to the Sensor.

For plain HTTP requests (non-CloudEvents), the event context attributes are
generated by Argo Events as before.
