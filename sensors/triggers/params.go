...
func ApplyResourceParameters(sensor *v1alpha1.Sensor, trigger *v1alpha1.Trigger, obj *unstructured.Unstructured) error {
	if trigger.ResourceParameters != nil && len(trigger.ResourceParameters) > 0 {
		jObj, err := obj.MarshalJSON()
		if err != nil {
			return err
		}
		jUpdatedObj, err := applyParams(jObj, trigger.ResourceParameters, extractEvents(sensor, trigger.ResourceParameters))
		if err != nil {
			return err
		}
...
func resolveParamValue(src *v1alpha1.TriggerParameterSource, events map[string]apicommon.Event) (string, error) {
	if event, ok := events[src.Event]; ok {
		...
		if err != nil {
			fmt.Printf("failed to parse the payload, using default value. err: %+v", err)
			if src.Value != nil {
				return *src.Value, nil
			}
			return "", errors.Wrap(err, "failed to render event payload as JSON")
		}
	...
	return "", errors.Errorf("unable to resolve '%s' parameter value. verify the path: '%s' is valid and/or set a default value for this param", src.Event, src.Path)
}
