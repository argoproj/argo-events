...
func (listener *EventListener) StartEventSource(eventSource *gateways.EventSource, eventStream gateways.Eventing_StartEventSourceServer) error {
	...
	if fileEventSource.Polling {
		if err := listener.listenEventsPolling(eventSource, channels); err != nil {
...
func (listener *EventListener) listenEventsPolling(eventSource *gateways.EventSource, channels *server.Channels) error {
	logger := listener.Logger.WithField(common.LabelEventSourceName, eventSource.Name)
		...
		for {
			...
			case event, ok := <-watcher.Event:
				if !ok {
					logger.Info("fs watcher has stopped")
					// watcher stopped watching file events
					logger.Errorf("fs watcher stopped for %s", eventSource.Name)
					return
				}
				// fwc.Path == event.Name is required because we don't want to send event when .swp files are created
				matched := false
			...
			case err := <-watcher.Error:
				logger.WithError(err).Errorf("failed to process %s", eventSource.Name)
				return
			case <-channels.Done:
				logger.Infoln("event source has been stopped")
				return
