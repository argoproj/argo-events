...
func (gc *GatewayConfig) WatchGatewayConfigMap(ctx context.Context, configManager func(config *ConfigData) error, configDeactivator func(config *ConfigData) error) (cache.Controller, error) {
...
func (gc *GatewayConfig) manageConfigurations(configActivator func(config *ConfigData) error, configDeactivator func(config *ConfigData) error, cm *corev1.ConfigMap) error {
	...
	for _, newConfigKey := range newConfigKeys {
		...
		// run configuration
		gc.Log.Info().Str("config-key", newConfig.Src).Msg("activating configuration...")
	...
	for _, staleConfigKey := range staleConfigKeys {
		...
		if err == nil {
			gc.Log.Info().Str("config", staleConfig.Src).Msg("configuration deactivated.")
			delete(gc.registeredConfigs, staleConfigKey)
		} else {
			gc.Log.Warn().Str("config", staleConfig.Src).Msg("failed to deactivate the configuration.")
		}
	}
	return nil
}
