...
func normalizeURI(refPath, base string) string {
	...
	r := k8sspec.MustCreateRef(refURL.String())
	if r.IsCanonical() {
...
//
func denormalizeRef(ref *k8sspec.Ref, originalRelativeBase, id string) k8sspec.Ref {
...
func rebase(ref *k8sspec.Ref, v *url.URL, notEqual bool) (k8sspec.Ref, bool) {
	var newBase url.URL
	...
	return k8sspec.MustCreateRef(newBase.String()), true
}

// normalizeRef canonicalize a Ref, using a canonical relativeBase as its absolute anchor
func normalizeRef(ref *k8sspec.Ref, relativeBase string) *k8sspec.Ref {
	r := k8sspec.MustCreateRef(normalizeURI(ref.String(), relativeBase))
	return &r
}
