...
type BitbucketEventSource struct {
	...
	// +required
	Auth *BitbucketAuth `json:"auth" protobuf:"bytes,1,name=auth"`
	// ApiBaseURL is the base URL for the Bitbucket API.
	// +required
	ApiBaseURL string `json:"apiBaseURL,omitempty" protobuf:"bytes,2,opt,name=apiBaseURL"`
	// Owner of the repository.
	// +required
	Owner string `json:"owner" protobuf:"bytes,3,name=owner"`
	// RepoSlug is a URL-friendly version of a repository name, automatically generated by Bitbucket for use in the URL.
	// +required
	RepoSlug string `json:"repoSlug" protobuf:"bytes,4,name=repoSlug"`
	// Webhook holds configuration for a REST endpoint
	// +required
	Webhook *WebhookContext `json:"webhook" protobuf:"bytes,5,name=webhook"`
	// Events this webhook is subscribed to.
	// +required
	Events []string `json:"events" protobuf:"bytes,6,name=events"`
	// Metadata holds the user defined metadata which will passed along the event payload.
	// +optional
	Metadata map[string]string `json:"metadata,omitempty" protobuf:"bytes,7,rep,name=metadata"`
}
