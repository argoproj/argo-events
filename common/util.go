...
// CopyDeploymentSpecTemplate is used to copy exsired fields from a src podtemplatespc to a dest.
func CopyDeploymentSpecTemplate(dest, src *v1.PodTemplateSpec) *v1.PodTemplateSpec {
	if src == nil && dest == nil {
		return nil
	}
	if dest == nil {
	...
	if len(src.Spec.ServiceAccountName) > 0 {
		result.Spec.ServiceAccountName = src.Spec.ServiceAccountName
	}
	if len(src.Spec.Volumes) > 0 { // replace with all the volumes in src
		result.Spec.Volumes = src.Spec.Volumes
	}
	if len(src.Spec.Containers) > 0 { // replace with all the containers in src
		result.Spec.Containers = src.Spec.Containers
	}
	return result
}

// CopyServiceSpec is used to copy exsired src spec to the dest
func CopyServiceSpec(dest, src *v1.Service) *v1.Service {
	if src == nil && dest == nil {
		return nil
	}
	if dest == nil {
	...
	if len(src.Spec.Type) > 0 {
		result.Spec.Type = src.Spec.Type
	}
	if len(src.Spec.Ports) > 0 { // replace all the ports
		result.Spec.Ports = src.Spec.Ports
	}
	return result
}
